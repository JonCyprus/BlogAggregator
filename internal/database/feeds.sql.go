...
const getFeedsForPrint = `-- name: GetFeedsForPrint :many
SELECT feeds.name, feeds.url, users.name AS username
FROM feeds
INNER JOIN users ON feeds.user_id = users.id
`

type GetFeedsForPrintRow struct {
	Name     string
	Url      string
	Username string
}

func (q *Queries) GetFeedsForPrint(ctx context.Context) ([]GetFeedsForPrintRow, error) {
	rows, err := q.db.QueryContext(ctx, getFeedsForPrint)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetFeedsForPrintRow
	for rows.Next() {
		var i GetFeedsForPrintRow
